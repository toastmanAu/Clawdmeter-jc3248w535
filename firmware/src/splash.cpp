...
void splash_pick_for_current_rate() {
    if (SPLASH_ANIM_COUNT > 0) {
        cur_anim = random(SPLASH_ANIM_COUNT);
        cur_frame = 0;
...
static void render_frame(const uint8_t *cells, const uint16_t *palette) {
    ...
    for (int gy = 0; gy < GRID; gy++) {
        ...
        for (int dy = 0; dy < CELL; dy++) memcpy(&canvas_buf[(gy * CELL + dy) * CANVAS_W], row, CANVAS_W * 2);
    }
...
void splash_init(lv_obj_t *parent) {
    ...
    lv_obj_set_size(splash_container, 480, 320);
...
void splash_tick(void) {
    ...
    if (millis() - last_pick_ms >= 15000) splash_pick_for_current_rate();
...
void splash_show(void) { if (splash_container) lv_obj_clear_flag(splash_container, LV_OBJ_FLAG_HIDDEN); splash_pick_for_current_rate(); active = true; }
void splash_hide(void) { if (splash_container) lv_obj_add_flag(splash_container, LV_OBJ_FLAG_HIDDEN); active = false; }
